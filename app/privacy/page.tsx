  ...
  return (
    <main className="legal-shell">
      ...
      <article id="privacy">
        ...
        <p>Paper Picture offers optional sign-in with ChatGPT, Google, or GitHub to recognize you. Your profile and saved games are private: there is no public profile, leaderboard, social graph, or advertising tracker. You can also play without signing in.</p>
        <h2>What is stored</h2>
        <p>When you sign in, the chosen provider supplies a verified email address and may supply a display name. We store a display name, a one-way keyed identifier derived from your normalized email, collection version, selected game mode, randomized paper order, game timestamps, selected answer indexes, figures viewed, correctness, assisted-round status, and scores. Feedback may include its category, rating, related paper, and the text you submit. We do not store your raw sign-in email or provider access or refresh tokens in the gameplay database.</p>
        ...
        <h2>Retention</h2>
        <p>Unfinished game sessions are removed after 7 days. Submitted feedback is removed after 365 days. Aggregate operational counters are removed after 90 days. Completed games and your display name remain until you delete the profile so your history stays available. Short-lived rate-limit counters are removed after approximately 2 days. Google and GitHub sign-ins use a secured site session cookie that can remain valid for up to 30 days.</p>
        <h2>Sign-in providers</h2>
        <p>ChatGPT, Google, and GitHub process the information needed to authenticate you under their own terms and privacy notices. Paper Picture requests only basic identity information needed for sign-in. Provider access tokens are used only during the sign-in exchange and are not copied into your Paper Picture profile or gameplay records. If two supported providers return the same normalized verified email, they open the same private Paper Picture profile.</p>
        <h2>Your controls</h2>
        ...
        <p className="legal-version">Privacy notice version: 13 July 2026 · revision 5</p>
      </article>
    </main>
  );
}
