# Open Graphics Collection 01 — Rights Audit

Reviewed: 12 July 2026

## Inclusion rule

An item is playable only when its article page explicitly states CC BY 4.0, its selected figure carries no separate exclusion or third-party credit, its publication metadata and author affiliation are verified on the publication page, and its attribution record is complete in `data/papers.ts`.

The game stores the publisher-provided image without editing it. Responsive display sizing is performed by the browser.

## Approved papers

### Goal-adaptive Meshing of Isogeometric Kirchhoff–Love Shells

- DOI: https://doi.org/10.1007/s00366-024-01958-4
- Publication page: https://link.springer.com/article/10.1007/s00366-024-01958-4
- License: CC BY 4.0
- License evidence: the Rights and permissions section states that the article is CC BY 4.0 and included images are covered unless separately credited.
- Figures: 11, 15, and 13
- Caption credit check: no separate third-party credit is attached to either selected caption.
- Affiliation check: H. M. Verhelst is listed with Delft University of Technology, The Netherlands.
- Status: approved

### Functional maps regularization for high quality mesh morphing applied to shape registration in computed tomography

- DOI: https://doi.org/10.1007/s00366-025-02243-8
- Publication page: https://link.springer.com/article/10.1007/s00366-025-02243-8
- License: CC BY 4.0
- License evidence: the Rights and permissions section states that the article is CC BY 4.0 and included images are covered unless separately credited.
- Figures: 3, 6, and 4
- Caption credit check: no separate third-party credit is attached to either selected caption.
- Authorship evidence: the Contributions section states that Amelia Ferhat generated the figures and meshes.
- Affiliation check: Amelia Ferhat’s first listed affiliation is MINES ParisTech, PSL University, France.
- Status: approved

### Topology-inclusive aerodynamic shape optimisation using a cellular automata parameterisation

- DOI: https://doi.org/10.1007/s00158-024-03916-6
- Publication page: https://link.springer.com/article/10.1007/s00158-024-03916-6
- License: CC BY 4.0
- License evidence: the Rights and permissions section states that the article is CC BY 4.0 and included images are covered unless separately credited.
- Figures: 2, 28, and 26
- Caption credit check: no separate third-party credit is attached to either selected caption.
- Affiliation check: M. J. Wood is listed with the Department of Aerospace Engineering, University of Bristol, United Kingdom.
- Status: approved

## Excluded source types

- Default arXiv distribution licenses
- CC BY-ND publications requiring extracted or modified figures
- Papers whose selected figures carry a separate third-party credit
- Merely free-to-read or open-access papers without an explicit reuse license
- Project-page images without explicit reuse permission
